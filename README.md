...
* [`src/city_encounters/`](src/city_encounters/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/city_encounters/core.cljs): contains the SPA entry point, `init`
* [`test/city_encounters/`](test/city_encounters/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `city-encounters` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/city_encounters/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns city-encounters.example
  (:require [city-encounters.config :as config])
